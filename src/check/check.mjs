...
import {
  ...
  notify,
  localizeHtmlPage,
} from "../shared/modules/chrome_utils.mjs";

async function getTargetURL() {
  return (await chrome.storage.local.get(["url"])).url || config.target_url;
}
/**
 * Check notifications
 */
async function checkNotification() {
  await fetch(await getTargetURL())
    .then(async (res) => {
      ...
      document.getElementById("latest_check_time").innerText =
...
function getNotificationIdentifierList(html) {
  ...
  for (const element of elements) {
    ...
    let title = element
      .getElementsByClassName("markdown-title")[0]
      .innerText.trim();
...
chrome.notifications.onClicked.addListener(async (notificationId) => {
  const targetTabId = await searchTargetTabId(await getTargetURL());

  if (null === targetTabId) {
    // Open a new tab to display the page
    chrome.tabs.create({ url: getTargetURL() });
  } else {
